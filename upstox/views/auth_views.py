...
def get_token(code):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            error_details = ", ".join(
                [f"{error['errorCode']}: {error['message']}" for error in error_message]
            )
...
def logout(request):
    ...
    logout_url = "https://api.upstox.com/v2/login/logout"
    url_params = {"access_token": request.session["access_token"]}
    try:
        response = requests.post(logout_url, url_params)
        if response.status_code == 200:
            return HttpResponseRedirect("http://localhost:3000/")
        else:
            return HttpResponse("Logout failed", status=response.status_code)
    except requests.exceptions.RequestException as e:
        return HttpResponse(f"Request failed: {str(e)}", status=500)